...
# ----- Directory ----- #
dir_fig = '/data/jlee/DATA/Gemini/Programs/GS-2019A-Q-214/analysis/diagram/linefits/'
diG = '/data/jlee/DATA/Gemini/Programs/GS-2019A-Q-214/redux6_700/'
dir_lines = '/data/jlee/DATA/Gemini/Programs/GS-2019A-Q-214/analysis/lines2/'
...
hdr1 = fits.getheader(diG+'cstxeqxbrgS20190304S0134_3D.fits', ext=0)
...
# ----- START: Flux maps ----- #
for l in np.arange(len(emi_lines)):
    if (emi_lines[l] == 'Hbeta'):
        exec("snr_cnd = ((Halpha_snr_2D < 3.0) | (Halpha_snrpix_2D < 5.0) | ("+emi_lines[l]+"_snr_2D < 1.0))")
    else:
    ...
    ax.arrow(x0, y0-0.025, -L*np.cos(theta0), L*np.sin(theta0), width=0.06,
             head_width=0.18, head_length=0.18, fc='blueviolet', ec='blueviolet', alpha=0.9)
    ax.text(-3.30, 1.30, 'E', fontsize=15.0, fontweight='bold', color='blueviolet')
    ax.text(-2.25, 2.10, 'N', fontsize=15.0, fontweight='bold', color='blueviolet')
...
for l in np.arange(len(emi_lines)):
    ...
    # rchisq_cnd = (Halpha_rchisq_2D > 50.)
    # flx_cnd = (Halpha_flux_2D < flx50)
    # zero_cnd = (snr_cnd | sig_cnd | rchisq_cnd | flx_cnd)
...
snr_cnd = ((Halpha_snr_2D < 3.0) | (Halpha_snrpix_2D < 5.0) | (Hbeta_snr_2D < 1.0))
# sig_cnd = (Halpha_sigma_2D < lsig_llim)
rchisq_cnd = (Halpha_rchisq_2D > 50.)
flx_cnd = (Halpha_flux_2D < flx50)
zero_cnd = (snr_cnd | rchisq_cnd)
...
cax = divider.append_axes("right", size="5%", pad=0.05)
cb = plt.colorbar(im, cax=cax)
cb.set_label(r'Star formation rate [$M_{\odot}~{\rm yr^{-1}~kpc^{-2}}$]',
             size=12.0, labelpad=15.0)
cb.ax.tick_params(labelsize=12.0)
...
def cal_fwm_logOH(regfile):
    metal = copy.deepcopy(logOH)
...
# 1. HST
logOH_disk_hst, logOH_tail_hst = cal_fwm_logOH("HST_boundary_1sig_transformed.reg")
print(f"log OH (HST) : {logOH_disk_hst:.2f} + {logOH_tail_hst:.2f}")

# 2. Gemini
logOH_disk_gem, logOH_tail_gem = cal_fwm_logOH("GMOS_boundary_1sig.reg")
print(f"log OH (Gem) : {logOH_disk_gem:.2f} + {logOH_tail_gem:.2f}")
...
snr_cnd = ((NII6584_snr_2D < 3.0) | (Halpha_snr_2D < 3.0) | (Halpha_snrpix_2D < 5.0) | \
...
for p in np.arange(len(grid_params)):
    ...
    if (prior_func == "Gaussian"):
        gauss_lo = np.exp(-((all_values-cut_lo)/0.2)**2 / 2.)
        gauss_hi = np.exp(-((all_values-cut_hi)/0.2)**2 / 2.)
